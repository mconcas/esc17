...
int main(int argc, char **argv)
{
  ...
  int i,j, iters;
  double start_time, elapsed_time, pr1_start, pr2_start;
...
// }
// }
  printf("Time elapsed in loading all data: %lf s \n", omp_get_wtime() - pr2_start);
...
//
  ...
  while ((conv > TOLERANCE) && (iters<MAX_ITERS))
  {
    ...
    clerr = clEnqueueNDRangeKernel(
                ...
                ko_jacobi,    // the kernel object
                1,  NULL,     // num of dims for NDRange and index-value-offset  
                global,       // the size of the NDRange for each dim
                NULL,         // workgroup size in each dim (NULL = let system choose)
                0,            // num of events kernel will wait on 
                NULL,         // list of events to wait on
                NULL);        // events generated by this kernel
