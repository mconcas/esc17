...
int main(int argc, char* argv[])
{
  ...
  auto dirp = std::unique_ptr<DIR, std::function<int(DIR*)>>{opendir(name.c_str()), closedir};
  // int closedir(DIR *dirp);
  // int readdir_r(DIR *dirp, struct dirent *entry, struct dirent **result);
  // auto direntp = std::make_unique<dirent>(readdir_r(dirp));
